...
export default function Perfil() {
  ...
  useEffect(()=>{
    ...
    const completarDatosPersonales = () => {
      ...
      const {telefono,nombre_tutor,tel_tutor,institucion, sexo} = datosPersonales
  ...
  return (
    <main className="">
      ...
      <svg className="mt-8 w-32 h-32 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill={sexo === 'hombre' ? '#107bcc' : sexo === 'mujer'? '#ff5cab' : ''} viewBox="0 0 20 20">
    <path d="M10 0a10 10 0 1 0 10 10A10.011 10.011 0 0 0 10 0Zm0 5a3 3 0 1 1 0 6 3 3 0 0 1 0-6Zm0 13a8.949 8.949 0 0 1-4.951-1.488A3.987 3.987 0 0 1 9 13h2a3.987 3.987 0 0 1 3.951 3.512A8.949 8.949 0 0 1 10 18Z"/>
  </svg>
      <h1 className="text-4xl  text-blue-850">Perfil</h1>
      <h2 className="text-gray-400 text-xl mx-5 xl:mx-0">Al completar la información de tu perfil podrás solicitar informes con los administradores!</h2>
      </div>
      ...
      <div className="flex flex-col lg:col-span-2">
          <label className="text-blue-950 font-semibold" htmlFor="nombre_tutor">Nombre Completo</label>
          <input
            type="text"
            defaultValue={nombre}
            disabled
        ...
        <div className="flex flex-col">
          <label className="text-blue-950 font-semibold" htmlFor="nombre_tutor">Nombre del tutor</label>
          <input
          defaultValue={nombre_tutor}
            type="text"
            {...Object.keys(datosPersonales).length !== 0 ? {disabled:true} : {disabled:false}}
        ...
        <div className="flex flex-col">
          ...
          defaultValue={tel_tutor}
            type="number"
            {...datosPersonales.tel_tutor > 0 ? {disabled:true} : {disabled:false}}
            placeholder="961 000 0000"
            className="p-3 rounded  border focus:outline-slate-300 text-slate-700"
        ...
        type="submit" 
        value='Enviar' 
        className={`lg:col-span-2 bg-sky-950 text-white p-3 font-semibold cursor-pointer rounded ${Object.keys(datosPersonales).length === 0 ? 'hover:bg-sky-900' : 'bg-slate-300'}`}
        {...Object.keys(datosPersonales).length === 0 ? {disabled:false} : {disabled:true}}
        />
      </form>
    </main>
  );
