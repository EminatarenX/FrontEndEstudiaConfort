...
export default function AdminHabitaciones() {
  ...
  const modify = () => {
    ...
    let capacidadParse = Number(capacidad)
    let precioParse = Number(precio)
    if ([descripcion, capacidad, ciudad, direccion, precio, estado].includes('')) {
      alerta('Todos los campos son obligatorios', 'error')
      return
    }
    ...
    <>
      ...
      <main className={`p-2 flex flex-col gap-5 `}>
        ...
        <div className="flex justify-center">
          {
          modificar ? (
            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={cancelar}>
              Cancelar
          </button>
          ):
        ...
        <article className={`h-400 bg-slate-200 p-5 animate-entrada ${!showCrear && 'hidden'}`}>
          <form
