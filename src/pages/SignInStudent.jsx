...
const SignInStudent = () => {
  ...
  return (
    <main className='flex justify-center flex-col xl:flex-row bg-degradado pt-10 pb-20 gap-20'>
      ...
      <form onSubmit={handleSubmit} className='animate-entrada  px-5 flex flex-col gap-2'>
      ...
      {error.error && <Alerta error={error.msj}/>}
        ...
        <input className='border border-gray-300 p-3 text-xs rounded transition-all focus:outline-sky-500 active:scale-105' type="password" id='password' placeholder='Contraseña'
          onChange={e=> setPassword(e.target.value)}
        />
        <label className='text-xs capitalize text-white' htmlFor="repetir">Confirmar contraseña</label>
        <input className='border border-gray-300 p-3 text-xs rounded transition-all focus:outline-sky-500 active:scale-105' type="password" placeholder='Repetir contraseña' 
