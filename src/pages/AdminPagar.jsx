...
export default function AdminPagar() {
  ...
  useEffect(() => {
    ...
    const filtrados = listaPagos.filter(pago => {
      ...
      const nombreMesPago = obtenerNombreMes(fechaPago.getMonth()).toLowerCase();
  ...
  return (
    <div className="bg-slate-100 m-10 p-5 flex flex-col items-center gap-5">
      ...
      <select  className='text-center w-[250px] py-2' onChange={e => setFiltro(e.target.value)}>
        <option value="">Todos</option>
        {listaMeses().map((mes, index)=> (
          <option key={index} value={mes.toLowerCase()}>{mes.toUpperCase()}</option>
        ))}
       </select>
      
      {filtro.length === 0 ? listaPagos.map(pago => (
