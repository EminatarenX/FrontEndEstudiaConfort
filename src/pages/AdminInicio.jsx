...
function AdminInicio() {
  const { obtenerEstudiantes, estudiantes, cargando, rechazarSolicitud, cambiarEstado, modal, setModal, cambiarEstadoRenta } = useEstudiantes()
  const {auth} = useAuth()
  ...
  return (
    <>
      {modal && (
        <div className="fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-80 z-50 flex justify-center items-center animate-entrada ">
          <button onClick={() => {
            setModal(false)
            setEstudiante({})
            }} className="fixed top-3 right-10 xl:top-10 xl:right-10 text-white font-bold text-[40px]">X</button>
          <div className="bg-white rounded-xl p-1 xl:p-8 flex flex-col">
            <h1 className="text-2xl text-center mb-2 text-gray-800">Datos del Estudiante</h1>
            ...
            </div>
            {
              estudianteSeleccionado.renta === 'pendiente' && estudianteSeleccionado.estado === 'rentando' ? (
                <>
                <button onClick={()=> cambiarEstado(estudianteSeleccionado.solicitud_id)} className="bg-blue-700 p-2 rounded text-white mt-2 hover:bg-blue-500 transition-all">{estudianteSeleccionado.estado === 'pendiente' ? 'Aceptar solicitud' : 'Mover a solicitudes' }</button>
                <button onClick={cambiarPago} className="bg-green-700 p-2 rounded text-white mt-2 hover:bg-green-500 transition-all ">Acreditar pago</button>
                </>
                ) : estudianteSeleccionado.estado === 'pendiente' ?(
                  <>
                  <button onClick={()=> cambiarEstado(estudianteSeleccionado.solicitud_id)} className="bg-blue-700 p-2 rounded text-white mt-2 hover:bg-blue-500 transition-all">{estudianteSeleccionado.estado === 'pendiente' ? 'Aceptar solicitud' : 'Mover a solicitudes' }</button>
                    <button onClick={rechazar} className="bg-red-700 p-2 rounded text-white mt-2 hover:bg-red-500 transition-all ">Rechazar solicitud</button>
                </>)
                : (
                  <>
                  <button onClick={()=> cambiarEstado(estudianteSeleccionado.solicitud_id)} className="bg-blue-700 p-2 rounded text-white mt-2 hover:bg-blue-500 transition-all">{estudianteSeleccionado.estado === 'pendiente' ? 'Aceptar solicitud' : 'Mover a solicitudes' }</button>
                    <button onClick={cambiarPago} className="bg-red-700 p-2 rounded text-white mt-2 hover:bg-red-500 transition-all ">Cambiar Estado de pago</button>
                </>
                )
              }
          </div>
      ...
      <main className="flex flex-col gap-5 p-2 mb-20">
        <h1 className="text-4xl mt-5 text-center">Bienvenido al panel de Administrador {nombre}</h1>
        ...
        <article className="flex bg-slate-300 gap-10 py-5 px-3 overflow-x-scroll scroll-smooth">
          <section className="flex flex-col bg-slate-100 min-w-[350px] xl:min-w-[500px] border rounded-lg h-[600px] xl:h-[450px]">
            <div className="flex justify-between bg-white p-2 shadow-lg m-1 rounded">
              ...
              <span className="text-slate-500">{estudiantes.filter(estudiante => estudiante.estado === 'pendiente').length} solicitudes</span>
            </div>
            <div className="p-2">
              {estudiantes.length === 0 ? <h1 className="text-slate-800 ">{cargando ? "Cargando ..." : "No hay registros"}</h1> : (
                estudiantes.map(estudiante => {
                  if (estudiante.estado === 'pendiente') return (
                    <EstudianteEnTabla
          ...
          <section className="flex flex-col bg-slate-100 min-w-[350px] xl:min-w-[500px] border rounded-lg h-[600px] xl:h-[450px]">
            <div className="flex justify-between p-2 bg-white shadow-lg m-1 rounded">
              ...
              <span className="text-slate-500">{estudiantes.filter(estudiante => estudiante.estado === 'rentando').length} estudiantes</span>
            </div>
            <div className="p-2">
              {estudiantes.filter(estudiante => estudiante.estado === 'rentando').length === 0 ? <h1 className="text-slate-800 ">{cargando ? "Cargando ..." : "No hay registros"}</h1> : (
                estudiantes.map(estudiante => {
          ...
          <section className="flex flex-col bg-slate-100 min-w-[350px] xl:min-w-[500px] border rounded-lg h-[600px] xl:h-[450px]">
            <div className="flex justify-between p-2 shadow-lg m-1 rounded">
              ...
              <span className="text-slate-500">{estudiantes.filter(estudiante => estudiante.estado === 'rentando' && estudiante.renta === 'pendiente').length} pagos</span>
            </div>
            <div className="p-2">
              {estudiantes.filter(estudiante => estudiante.estado === 'rentando' && estudiante.renta === 'pendiente').length === 0 ? <h1 className="text-slate-800 ">{cargando ? "Cargando ..." : "No hay registros"}</h1> : (
                estudiantes.map((estudiante, index) => {
                  if (estudiante.estado === 'rentando' && estudiante.renta === 'pendiente') return (
                    <EstudianteEnTabla
                      key={estudiante.solicitud_id}
                      nombre={estudiante.nombre}
                      institucion={estudiante.institucion}
          ...
          <section className="flex flex-col bg-slate-100 min-w-[350px] xl:min-w-[500px] border rounded-lg h-[600px] xl:h-[450px]">
            <div className="flex justify-between p-2 shadow-lg m-1 rounded">
              ...
              <span className="text-slate-500">{estudiantes.filter(estudiante => estudiante.estado === 'rentando' && estudiante.renta === 'pagado').length} realizados</span>
            </div>
            <div className="p-2">
              {estudiantes.filter(estudiante => estudiante.estado === 'rentando' && estudiante.renta === 'pagado').length === 0 ? <h1 className="text-slate-800 ">{cargando ? "Cargando ..." : "No hay registros"}</h1> : (
                estudiantes.map(estudiante => {
                  if (estudiante.estado === 'rentando' && estudiante.renta === 'pagado') return (
                    <EstudianteEnTabla
                      key={estudiante.solicitud_id}
                      nombre={estudiante.nombre}
                      institucion={estudiante.institucion}
        ...
        { buscar.length === 0 ? <h1 className="text-center text-gray-500">Los estudiantes buscados apareceran aqui</h1>:
              estudiantesBuscados.map(estudiante => {
                if(estudiante.estado === 'rentando') return(
                    ...
                    <div className="shadow-xl p-5 flex flex-col items-center m-10 hover:scale-105 transition-all cursor-pointer animate-entrada bg-white"
                    onClick={()=> {
                      setModal(true)
                      setEstudiante(estudiante)
                    }}
                    >
                      <svg className="w-20 h-20 mx-4  text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill={estudiante.sexo === 'hombre' ? '#107bcc' : '#ff5cab'} viewBox="0 0 14 18">
                        <path d="M7 9a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9Zm2 1H5a5.006 5.006 0 0 0-5 5v2a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2a5.006 5.006 0 0 0-5-5Z" />
                      </svg>
                      <p className="text-4xl text-center">{estudiante.nombre}</p>
                      <p className="text-sm text-gray-500">{estudiante.institucion}</p>
